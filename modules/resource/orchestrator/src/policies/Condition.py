...
class Condition():
	...
	def evaluate(self, metaObj,resolver):
		print "Policies > evaluate condition"
                print "Policies > metaObj: ", metaObj
		print "Policies > resolver: ", resolver
		#Retrieve operand value
		rightValue=None
		leftValue=None
		print "Policies > lefOp: ", self._leftOperand
		print "Policies > rightOp: ", self._rightOperand

		if Condition.isCondition(self._leftOperand):
			print "Policies > left operand is condition"
			leftValue = self._leftOperand.evaluate(metaObj,resolver)
			print "Policies > leftValue: ", leftValue
			rightValue = self._rightOperand.evaluate(metaObj,resolver)	
			print "Policies > rightValue: ", rightValue
		else:
			...
			if Collection.isCollection(self._rightOperand):
				...
				try:
					...
					print "Beep 4. rightValue: ", rightValue
				except:
					print "Beep 5"
					rightValue =  self._rightOperand
			...
			#Try to first resolve (allow string based comparison)
			try:
				print "Policies > leftop: ", self._leftOperand
				leftValue = resolver.resolve(self._leftOperand,metaObj)
				print "Beep 6. LeftValue: ", leftValue
			except:
				print "Beep 7"
				leftValue =  self._leftOperand


		print "type leftv: ", type(leftValue), leftValue
		print "type rightv: ", type(rightValue), rightValue
		...
		print "Policies > operator: ", self._operator
		if self._operator == "=":
